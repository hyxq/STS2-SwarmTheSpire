...
namespace SwarmTheSpire.Powers
{
    public sealed class NeuroPlayPower : SwarmPowerTemplate
    {
        ...
        public override Task BeforeCardPlayed(CardPlay cardPlay)
        {
            ...
            GetInternalData<Data>().AmountsForPlayedCards.Add(cardPlay.Card, Amount * 2);
            return Task.CompletedTask;
        }
        ...
        public override async Task AfterCardPlayed(PlayerChoiceContext context, CardPlay cardPlay)
        {
            ...
            await CreatureCmd.GainBlock(Owner, value, ValueProp.Unpowered, null, fast: true);
        }
        ...
        public override async Task AfterAutoPrePlayPhaseEnteredLate(PlayerChoiceContext choiceContext, Player player)
        {
            ...
            using (CardSelectCmd.PushSelector(new VakuuCardSelector()))
            {
                for (; cardsPlayed < MaxCardsToPlay; cardsPlayed++)
                {
                    ...
                    var playable = PileType.Hand.GetPile(Owner.Player).Cards.Where(c => c.CanPlay()).ToList();
                    if (playable.Count == 0)
                        break;
                    ...
                    var card = skills.Count > 0 ? rng.NextItem(skills) : rng.NextItem(playable);
            ...
            TalkCmd.Play(
                cardsPlayed >= MaxCardsToPlay
                    ? new LocString("cards", "SWARM_THE_SPIRE_CARD_NEURO_PLAY.warning")
                    : new LocString("cards", "SWARM_THE_SPIRE_CARD_NEURO_PLAY.approval"),
                Owner,
                VfxColor.Purple);
        }
